local c = require('umbra.colors')
local cfg = vim.g.umbra_config
local util = require("umbra.util")
...
if vim.api.nvim_call_function("has", { "nvim-0.8" }) == 1 then
    hl.treesitter = {
        ...
        ["@boolean"] = colors.Blue,
        ["@character"] = colors.Orange,
        ["@comment"] = {fg = c.comment or c.grey, fmt = cfg.code_style.comments},
        ...
        ["@constant"] = {fg = c.constant or c.orange, fmt = cfg.code_style.constants},
        ["@constant.builtin"] = {fg = c.blue, fmt = cfg.code_style.constants},
        ["@constant.macro"] = {fg = c.orange, fmt = cfg.code_style.constants},
        ["@constructor"] = {fg = c.constructor or c.orange},
        ...
        ["@none"] = colors.Fg,
        ["@number"] = {fg = c.number or c.orange},
        ["@number.float"] = {fg = c.float or c.number or c.orange},
        ["@operator"] = colors.Fg,
        ["@parameter.reference"] = colors.Fg,
        ["@property"] = colors.Cyan,
        ["@punctuation.delimiter"] = colors.Fg,
        ["@punctuation.bracket"] = colors.Fg,
        ["@string"] = {fg = c.string or c.green, fmt = cfg.code_style.strings},
        ["@string.regexp"] = {fg = c.orange, fmt = cfg.code_style.strings},
        ["@string.escape"] = {fg = c.escape or c.red, fmt = cfg.code_style.strings},
...
hl.plugins.indent_blankline = {
    ...
    IblScope = { fg = c.ibl_scope or c.light_grey, fmt = "nocombine" },
}
...
function M.setup()
        ...
        if color_name:sub(1, 1) == '$' then
            ...
            if not color_name then
                vim.schedule(function()
                    vim.notify('umbra.nvim: unknown color "' .. name .. '"', vim.log.levels.ERROR, { title = "umbra.nvim" })
                end)
                return ""
            end
        end
    ...
    for group_name, group_settings in pairs(vim.g.umbra_config.highlights) do
